...
import uz.gita.photoeditorpro.databinding.ItemEmojiBinding
import uz.gita.photoeditorpro.databinding.ScreenEditorBinding
import uz.gita.photoeditorpro.presentation.ui.editor.viewmodel.EditorViewModel
import uz.gita.photoeditorpro.presentation.ui.editor.viewmodel.impl.EditorViewModelImpl
import uz.gita.photoeditorpro.utils.EventBass
import uz.gita.photoeditorpro.utils.visible
...
//    private val navArgs: EditorScreenArgs by navArgs()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val icons = listOf(R.drawable.ic_crop, R.drawable.ic_stiker, R.drawable.ic_filter, R.drawable.ic_text)

        viewModel.addEmojiFlow.onEach {
            addEmojiToContainer(it)
        }.launchIn(lifecycleScope)

        EventBass.emojiBassLiveData.observe(viewLifecycleOwner, eventBassObserver)
    ...
    val eventBassObserver = Observer<Int> {
        ...
        emojiBinding.btnRotate.setOnTouchListener { v, event ->
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    val a =
                        if ((startPositionOfRotation.x < event.x && startPositionOfRotation.y < event.y)
                            || (startPositionOfRotation.x == event.x && startPositionOfRotation.y < event.y)
                            || (startPositionOfRotation.x > event.x && startPositionOfRotation.y < event.y)
                            || (startPositionOfRotation.x > event.x && startPositionOfRotation.y == event.y)
                        ) {
                            1
                    ...
                    emojiBinding.root.rotation += (a * degree)
                }
                MotionEvent.ACTION_UP -> {
